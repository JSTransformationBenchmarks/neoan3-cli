...
const ascii = '\n' + '::::    ::: :::::::::: ::::::::      :::     ::::    :::  ::::::::  \n' + ':+:+:   :+: :+:       :+:    :+:   :+: :+:   :+:+:   :+: :+:    :+: \n' + ':+:+:+  +:+ +:+       +:+    +:+  +:+   +:+  :+:+:+  +:+        +:+ \n' + '+#+ +:+ +#+ +#++:++#  +#+    +:+ +#++:++#++: +#+ +:+ +#+     +#++:  \n' + '+#+  +#+#+# +#+       +#+    +#+ +#+     +#+ +#+  +#+#+#        +#+ \n' + '#+#   #+#+# #+#       #+#    #+# #+#     #+# #+#   #+#+# #+#    #+# \n' + '###    #### ########## ########  ###     ### ###    ####  ########  \n\n';
let concr = {
  getCurrentVersion: async function (pure) {
    let pack = JSON.parse(await fs_readFilePromise(__dirname + '/package.json', 'utf8'));
    ...
    if (typeof cmd !== 'undefined') {
      ...
      try {
        ...
        if (typeof onlineVersion.results !== 'undefined') {
          if (!stringHelper.analyzeVersions(localVersion, onlineVersion.results[0].package.version)) {
            console.log('/**************');
            console.log('*');
  ...
  newApp: function (name) {
    ...
    if (typeof name === 'undefined') {
      let rootWarning = 'You did not provide a name for this app. ' + 'neoan3 assumes you will run this app under the web-root. ' + 'If this is not the case, please stop here and rerun "neoan3 ' + 'new app DIRECTORY-NAME". Do you want to continue?';
      questions.push({
        name: 'root',
        message: rootWarning,
        type: 'confirm'
