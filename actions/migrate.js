...
const migrate = {
  ...
  processDown: function (credentials, distinct) {
    mysqlConnection.connect(credentials).then(connection => {
      ...
      migrateHelper.compare.getFullDb(credentials).then(() => {
        ...
        for (const [arrayIteratorIndex, model] of Object.keys(migrateHelper.compare.knownModels).entries()) {
          if (typeof distinct === 'undefined' || distinct === model) {
            console.log('writing model: ' + model);
            await this.writeJson(model);
          }
        }
  ...
  writeJson: async function (model) {
    await fs_writeFilePromise(dir + './model/' + model + '/migrate.json', JSON.stringify(migrateHelper.compare.knownModels[model]), function (err, outd) {
      if (err) {
        throw new Error(err);
      }
    });
