...
let fileCreator = {
  ...
  transformer: function (name, cType, answer) {
    ...
    if (transformer.check(name)) {
      let structure = transformer.produceStructure(name, "    ", "    ".repeat(3));
  ...
  template: function (fileType) {
    ...
    for (const [arrayIteratorIndex, fileEnding] of fileEndings.entries()) {
      file = dir + '_template/' + fileType + '.' + fileEnding;

      if (await fs_existsPromise(file)) {
        console.log('using %s-template...', fileEnding);
        templates[fileEnding] = await fs_readFilePromise(file, 'utf8');
      }
    }
    ...
    write: function (name) {
      ...
      fileEndings.forEach(fileEnding => {
        ...
        if (typeof templates[fileEnding] === 'undefined' && fileEnding === 'js') {
          ...
          content = stringHelper.embrace(templates[fileEnding], {
            name: name
          });
          ...
          fs.appendFile(targetFolder + (fileEnding === 'php' ? stringHelper.fucase(name) : casedName) + identifier + fileEnding, content, function (err) {
            if (err) throw err;
          });
        }
      });
  ...
  htmlView: function (name) {
    ...
    if (typeof template.html !== 'undefined') {
      content = stringHelper.embrace(template.html, {
        name: name
      });
    } else {
      content = stringHelper.embrace('<h1>{{name}}</h1>', {
        name: name
      });
    }
