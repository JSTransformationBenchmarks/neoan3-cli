...
const Publish = {
  ...
  async init(type, name) {
    ...
    this.fileOperations.createComposerShell(type, name).then(async composer => {
      let path = './' + type + '/' + name + '/';
      ...
      if (await fs_existsPromise(path)) {
        composer = this.fileOperations.parseDependencies(path, composer);
      }

      await fs_writeFilePromise('./' + type + '/' + name + '/composer.json', JSON.stringify(composer, null, 4), function (err, outd) {
        if (err) {
          throw new Error(err);
        }
      });
      let gitQuestions = [{
        ...
        name: 'github',
        type: 'confirm',
        message: 'neoan3-cli can try to set a remote on GitHub for you.' + ' Do you want that?',
        when: answers => {
